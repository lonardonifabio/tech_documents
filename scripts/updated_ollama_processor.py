#!/usr/bin/env python3
"""
Updated Ollama document processor with correct 400-character summary prompt
Uses DeepSeek AI model for document analysis
"""
...
class UpdatedOllamaDocumentProcessor:
    ...
    def analyze_document_with_ollama(self, text: str, filename: str) -> Dict:
        """Use Ollama with DeepSeek to analyze document content with the exact prompt specified"""
        if not text.strip():
        ...
        # Use the exact prompt as specified by the user
...
Respond only with valid JSON, no additional text."""
        ...
        try:
            ...
            logger.info(f"Raw Ollama response for {filename}: {response_text}")
    ...
    def process_document(self, filepath: Path) -> Dict:
        ...
        # Analyze with Ollama using the correct prompt
...
def main():
    ...
    processor = UpdatedOllamaDocumentProcessor(model_name)
    
    logger.info("Starting document processing with updated Ollama processor...")
    if force_reprocess:
